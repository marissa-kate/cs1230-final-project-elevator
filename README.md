...
Changes were refactoring the screenspace bloom pass and the color grading pass to output to textures to fbo rather than to the screen. Additionally, we had to create another FBO as screenspace bloom requires binding to pingpong FBOs to create blur. This, in turn, requires us to bind to another fbo (the composite fbo), where we store the textures for color grading and final blur + final color of the object. We then do this as our pipeline was running into errors after attempting to rebind to the previous fbo where we did our phong, geometry, and blur pass, as any new information we stored into the empty textures in the fbo resulted in black in the output (but could still easily sample information stored in textures during previous passes before being bound to the pingpong fbos). We then also stored the final oclor grading into the texture to generate depth of field, which requires the final color to be stored in a texture to generate mipmaps. We thus output to the screen after the depth of field pass, but it is possible to do so by passing in depth of field to the texture and then color grading samples from the final color stored during depth of field.

Additionally, we required this many FBOs, as the order in which we do our passes was less flexible; we must first do lighting to do bloom, then, and then we must have lighting calculations after bloom be stored in a texture for depth of field to generate mipmaps.
